...
####################################################################################
#SCZ
scz_mean <- mean_log_p[grep("^SCZ", rownames(mean_log_p)), 11:18]
scz_ldsc <- ldsc[grep("^SCZ", rownames(ldsc)), 11:18]
scz_magma <- magma[grep("^SCZ", rownames(magma)), 11:18]
rownames(scz_mean) <- gsub("[A-Za-z0-9]+-","", rownames(scz_mean))
colnames(scz_mean) <- gsub("Mean_Male_", "", colnames(scz_mean))
rownames(scz_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(scz_ldsc))
colnames(scz_ldsc) <- gsub("Mean_Male_", "", colnames(scz_ldsc))
rownames(scz_magma) <- gsub("[A-Za-z0-9]+-","", rownames(scz_magma))
colnames(scz_magma) <- gsub("Mean_Male_", "", colnames(scz_magma))
...
####################################################################################
#BD
bd_mean <- mean_log_p[grep("^BD", rownames(mean_log_p)), 11:18]
bd_ldsc <- ldsc[grep("^BD", rownames(ldsc)), 11:18]
bd_magma <- magma[grep("^BD", rownames(magma)), 11:18]
rownames(bd_mean) <- gsub("[A-Za-z0-9]+-","", rownames(bd_mean))
colnames(bd_mean) <- gsub("Mean_Male_", "", colnames(bd_mean))
rownames(bd_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(bd_ldsc))
colnames(bd_ldsc) <- gsub("Mean_Male_", "", colnames(bd_ldsc))
rownames(bd_magma) <- gsub("[A-Za-z0-9]+-","", rownames(bd_magma))
colnames(bd_magma) <- gsub("Mean_Male_", "", colnames(bd_magma))
...
####################################################################################
#MDD
mdd_mean <- mean_log_p[grep("^MDD", rownames(mean_log_p)), 11:18]
mdd_ldsc <- ldsc[grep("^MDD", rownames(ldsc)), 11:18]
mdd_magma <- magma[grep("^MDD", rownames(magma)), 11:18]
rownames(mdd_mean) <- gsub("[A-Za-z0-9]+-","", rownames(mdd_mean))
colnames(mdd_mean) <- gsub("Mean_Male_", "", colnames(mdd_mean))
rownames(mdd_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(mdd_ldsc))
colnames(mdd_ldsc) <- gsub("Mean_Male_", "", colnames(mdd_ldsc))
rownames(mdd_magma) <- gsub("[A-Za-z0-9]+-","", rownames(mdd_magma))
colnames(mdd_magma) <- gsub("Mean_Male_", "", colnames(mdd_magma))
...
####################################################################################
#INT
int_mean <- mean_log_p[grep("^Inte", rownames(mean_log_p)), 11:18]
int_ldsc <- ldsc[grep("^Inte", rownames(ldsc)), 11:18]
int_magma <- magma[grep("^Inte", rownames(magma)), 11:18]
rownames(int_mean) <- gsub("[A-Za-z0-9]+-","", rownames(int_mean))
colnames(int_mean) <- gsub("Mean_Male_", "", colnames(int_mean))
rownames(int_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(int_ldsc))
colnames(int_ldsc) <- gsub("Mean_Male_", "", colnames(int_ldsc))
rownames(int_magma) <- gsub("[A-Za-z0-9]+-","", rownames(int_magma))
colnames(int_magma) <- gsub("Mean_Male_", "", colnames(int_magma))
...
####################################################################################
#AD
ad_mean <- mean_log_p[grep("^AD2022", rownames(mean_log_p)), 11:18]
ad_ldsc <- ldsc[grep("^AD2022", rownames(ldsc)), 11:18]
ad_magma <- magma[grep("^AD2022", rownames(magma)), 11:18]
rownames(ad_mean) <- gsub("[A-Za-z0-9]+-","", rownames(ad_mean))
colnames(ad_mean) <- gsub("Mean_Male_", "", colnames(ad_mean))
rownames(ad_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(ad_ldsc))
colnames(ad_ldsc) <- gsub("Mean_Male_", "", colnames(ad_ldsc))
rownames(ad_magma) <- gsub("[A-Za-z0-9]+-","", rownames(ad_magma))
colnames(ad_magma) <- gsub("Mean_Male_", "", colnames(ad_magma))
...
####################################################################################
#oct
oct_mean <- mean_log_p[grep("^OCT", rownames(mean_log_p)), 11:18]
oct_ldsc <- ldsc[grep("^OCT", rownames(ldsc)), 11:18]
oct_magma <- magma[grep("^OCT", rownames(magma)), 11:18]
rownames(oct_mean) <- gsub("[A-Za-z0-9]+-","", rownames(oct_mean))
colnames(oct_mean) <- gsub("Mean_Male_", "", colnames(oct_mean))
rownames(oct_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(oct_ldsc))
colnames(oct_ldsc) <- gsub("Mean_Male_", "", colnames(oct_ldsc))
rownames(oct_magma) <- gsub("[A-Za-z0-9]+-","", rownames(oct_magma))
colnames(oct_magma) <- gsub("Mean_Male_", "", colnames(oct_magma))
...
####################################################################################
#an
an_mean <- mean_log_p[grep("^Anxiety", rownames(mean_log_p)), 11:18]
an_ldsc <- ldsc[grep("^Anxiety", rownames(ldsc)), 11:18]
an_magma <- magma[grep("^Anxiety", rownames(magma)), 11:18]
rownames(an_mean) <- gsub("[A-Za-z0-9]+-","", rownames(an_mean))
colnames(an_mean) <- gsub("Mean_Male_", "", colnames(an_mean))
rownames(an_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(an_ldsc))
colnames(an_ldsc) <- gsub("Mean_Male_", "", colnames(an_ldsc))
rownames(an_magma) <- gsub("[A-Za-z0-9]+-","", rownames(an_magma))
colnames(an_magma) <- gsub("Mean_Male_", "", colnames(an_magma))
...
####################################################################################
#HS
hs_mean <- mean_log_p[grep("^Hoarding", rownames(mean_log_p)), 11:18]
hs_ldsc <- ldsc[grep("^Hoarding", rownames(ldsc)), 11:18]
hs_magma <- magma[grep("^Hoarding", rownames(magma)), 11:18]
rownames(hs_mean) <- gsub("[A-Za-z0-9]+-","", rownames(hs_mean))
colnames(hs_mean) <- gsub("Mean_Male_", "", colnames(hs_mean))
rownames(hs_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(hs_ldsc))
colnames(hs_ldsc) <- gsub("Mean_Male_", "", colnames(hs_ldsc))
rownames(hs_magma) <- gsub("[A-Za-z0-9]+-","", rownames(hs_magma))
colnames(hs_magma) <- gsub("Mean_Male_", "", colnames(hs_magma))
...
####################################################################################
#TS
ts_mean <- mean_log_p[grep("^Tourette", rownames(mean_log_p)), 11:18]
ts_ldsc <- ldsc[grep("^Tourette", rownames(ldsc)), 11:18]
ts_magma <- magma[grep("^Tourette", rownames(magma)), 11:18]
rownames(ts_mean) <- gsub("[A-Za-z0-9]+-","", rownames(ts_mean))
colnames(ts_mean) <- gsub("Mean_Male_", "", colnames(ts_mean))
rownames(ts_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(ts_ldsc))
colnames(ts_ldsc) <- gsub("Mean_Male_", "", colnames(ts_ldsc))
rownames(ts_magma) <- gsub("[A-Za-z0-9]+-","", rownames(ts_magma))
colnames(ts_magma) <- gsub("Mean_Male_", "", colnames(ts_magma))
...
####################################################################################
#INS
ins_mean <- mean_log_p[grep("^Insomina", rownames(mean_log_p)), 11:18]
ins_ldsc <- ldsc[grep("^Insomina", rownames(ldsc)), 11:18]
ins_magma <- magma[grep("^Insomina", rownames(magma)), 11:18]
rownames(ins_mean) <- gsub("[A-Za-z0-9]+-","", rownames(ins_mean))
colnames(ins_mean) <- gsub("Mean_Male_", "", colnames(ins_mean))
rownames(ins_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(ins_ldsc))
colnames(ins_ldsc) <- gsub("Mean_Male_", "", colnames(ins_ldsc))
rownames(ins_magma) <- gsub("[A-Za-z0-9]+-","", rownames(ins_magma))
colnames(ins_magma) <- gsub("Mean_Male_", "", colnames(ins_magma))
...
####################################################################################
#su
su_mean <- mean_log_p[grep("^Suicide", rownames(mean_log_p)), 11:18]
su_ldsc <- ldsc[grep("^Suicide", rownames(ldsc)), 11:18]
su_magma <- magma[grep("^Suicide", rownames(magma)), 11:18]
rownames(su_mean) <- gsub("[A-Za-z0-9]+-","", rownames(su_mean))
colnames(su_mean) <- gsub("Mean_Male_", "", colnames(su_mean))
rownames(su_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(su_ldsc))
colnames(su_ldsc) <- gsub("Mean_Male_", "", colnames(su_ldsc))
rownames(su_magma) <- gsub("[A-Za-z0-9]+-","", rownames(su_magma))
colnames(su_magma) <- gsub("Mean_Male_", "", colnames(su_magma))
...
####################################################################################
#ED
ed_mean <- mean_log_p[grep("^Eating", rownames(mean_log_p)), 11:18]
ed_ldsc <- ldsc[grep("^Eating", rownames(ldsc)), 11:18]
ed_magma <- magma[grep("^Eating", rownames(magma)), 11:18]
rownames(ed_mean) <- gsub("[A-Za-z0-9]+-","", rownames(ed_mean))
colnames(ed_mean) <- gsub("Mean_Male_", "", colnames(ed_mean))
rownames(ed_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(ed_ldsc))
colnames(ed_ldsc) <- gsub("Mean_Male_", "", colnames(ed_ldsc))
rownames(ed_magma) <- gsub("[A-Za-z0-9]+-","", rownames(ed_magma))
colnames(ed_magma) <- gsub("Mean_Male_", "", colnames(ed_magma))
...
####################################################################################
#ADHD
adhd_mean <- mean_log_p[grep("^ADHD", rownames(mean_log_p)), 11:18]
adhd_ldsc <- ldsc[grep("^ADHD", rownames(ldsc)), 11:18]
adhd_magma <- magma[grep("^ADHD", rownames(magma)), 11:18]
rownames(adhd_mean) <- gsub("[A-Za-z0-9]+-","", rownames(adhd_mean))
colnames(adhd_mean) <- gsub("Mean_Male_", "", colnames(adhd_mean))
rownames(adhd_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(adhd_ldsc))
colnames(adhd_ldsc) <- gsub("Mean_Male_", "", colnames(adhd_ldsc))
rownames(adhd_magma) <- gsub("[A-Za-z0-9]+-","", rownames(adhd_magma))
colnames(adhd_magma) <- gsub("Mean_Male_", "", colnames(adhd_magma))
...
####################################################################################
#ASD
asd_mean <- mean_log_p[grep("^ASD", rownames(mean_log_p)), 11:18]
asd_ldsc <- ldsc[grep("^ASD", rownames(ldsc)), 11:18]
asd_magma <- magma[grep("^ASD", rownames(magma)), 11:18]
rownames(asd_mean) <- gsub("[A-Za-z0-9]+-","", rownames(asd_mean))
colnames(asd_mean) <- gsub("Mean_Male_", "", colnames(asd_mean))
rownames(asd_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(asd_ldsc))
colnames(asd_ldsc) <- gsub("Mean_Male_", "", colnames(asd_ldsc))
rownames(asd_magma) <- gsub("[A-Za-z0-9]+-","", rownames(asd_magma))
colnames(asd_magma) <- gsub("Mean_Male_", "", colnames(asd_magma))
