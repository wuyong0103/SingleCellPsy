...
#Figure 4A
#读取数据
ldsc <- read.table("../LDSC-CellType/LDSC_Type.txt", header = TRUE, row.names = 1, sep = "\t")
magma <- read.table("../MAGMA-CellType/MAGMA_Type.txt", header = TRUE, row.names = 1, sep = "\t")
...
####################################################################################
#SCZ
scz_mean <- mean_log_p[grep("^SCZ", rownames(mean_log_p)), 20:27]
scz_ldsc <- ldsc[grep("^SCZ", rownames(ldsc)), 20:27]
scz_magma <- magma[grep("^SCZ", rownames(magma)), 20:27]
rownames(scz_mean) <- gsub("[A-Za-z0-9]+-","", rownames(scz_mean))
colnames(scz_mean) <- gsub("Mean_Female_", "", colnames(scz_mean))
rownames(scz_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(scz_ldsc))
colnames(scz_ldsc) <- gsub("Mean_Female_", "", colnames(scz_ldsc))
rownames(scz_magma) <- gsub("[A-Za-z0-9]+-","", rownames(scz_magma))
colnames(scz_magma) <- gsub("Mean_Female_", "", colnames(scz_magma))
...
marker_matrix <- marker_matrix[row_type_name,]
...
# 绘制热图
scz <- Heatmap(scz_mean[row_type_name,], cluster_rows = FALSE,cluster_columns = FALSE,
               name = "Mean(-log10(P))",col = colorRamp2(c(0, 2, 4, 6, 8), c("#F7FCF0", "#CCEBC5", "#7BCCC4", "#2B8CBE", "#084081")), 
               width = ncol(scz_mean)*unit(6, "mm"), 
               height = nrow(scz_mean)*unit(6, "mm"),column_title = "SCZ cell type",
               cell_fun = text_fun)

####################################################################################
#BD
bd_mean <- mean_log_p[grep("^BD", rownames(mean_log_p)), 20:27]
bd_ldsc <- ldsc[grep("^BD", rownames(ldsc)), 20:27]
bd_magma <- magma[grep("^BD", rownames(magma)), 20:27]
rownames(bd_mean) <- gsub("[A-Za-z0-9]+-","", rownames(bd_mean))
colnames(bd_mean) <- gsub("Mean_Female_", "", colnames(bd_mean))
rownames(bd_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(bd_ldsc))
colnames(bd_ldsc) <- gsub("Mean_Female_", "", colnames(bd_ldsc))
rownames(bd_magma) <- gsub("[A-Za-z0-9]+-","", rownames(bd_magma))
colnames(bd_magma) <- gsub("Mean_Female_", "", colnames(bd_magma))
...
# 绘制热图
bd <- Heatmap(bd_mean[row_type_name,], cluster_rows = FALSE,cluster_columns = FALSE,
              name = "Mean(-log10(P))",col = colorRamp2(c(0, 2, 4, 6, 8), c("#F7FCF0", "#CCEBC5", "#7BCCC4", "#2B8CBE", "#084081")), 
              width = ncol(bd_mean)*unit(6, "mm"), 
              height = nrow(bd_mean)*unit(6, "mm"),column_title = "BD cell type",
              cell_fun = text_fun)


####################################################################################
#MDD
mdd_mean <- mean_log_p[grep("^MDD", rownames(mean_log_p)), 20:27]
mdd_ldsc <- ldsc[grep("^MDD", rownames(ldsc)), 20:27]
mdd_magma <- magma[grep("^MDD", rownames(magma)), 20:27]
rownames(mdd_mean) <- gsub("[A-Za-z0-9]+-","", rownames(mdd_mean))
colnames(mdd_mean) <- gsub("Mean_Female_", "", colnames(mdd_mean))
rownames(mdd_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(mdd_ldsc))
colnames(mdd_ldsc) <- gsub("Mean_Female_", "", colnames(mdd_ldsc))
rownames(mdd_magma) <- gsub("[A-Za-z0-9]+-","", rownames(mdd_magma))
colnames(mdd_magma) <- gsub("Mean_Female_", "", colnames(mdd_magma))
...
# 绘制热图
mdd <- Heatmap(mdd_mean[row_type_name,], cluster_rows = FALSE,cluster_columns = FALSE,
               name = "Mean(-log10(P))",col = colorRamp2(c(0, 2, 4, 6, 8), c("#F7FCF0", "#CCEBC5", "#7BCCC4", "#2B8CBE", "#084081")),
               width = ncol(mdd_mean)*unit(6, "mm"), 
               height = nrow(mdd_mean)*unit(6, "mm"),column_title = "MDD cell type",
               cell_fun = text_fun)


####################################################################################
#INT
int_mean <- mean_log_p[grep("^Inte", rownames(mean_log_p)), 20:27]
int_ldsc <- ldsc[grep("^Inte", rownames(ldsc)), 20:27]
int_magma <- magma[grep("^Inte", rownames(magma)), 20:27]
rownames(int_mean) <- gsub("[A-Za-z0-9]+-","", rownames(int_mean))
colnames(int_mean) <- gsub("Mean_Female_", "", colnames(int_mean))
rownames(int_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(int_ldsc))
colnames(int_ldsc) <- gsub("Mean_Female_", "", colnames(int_ldsc))
rownames(int_magma) <- gsub("[A-Za-z0-9]+-","", rownames(int_magma))
colnames(int_magma) <- gsub("Mean_Female_", "", colnames(int_magma))
...
# 绘制热图
int <- Heatmap(int_mean[row_type_name,], cluster_rows = FALSE,cluster_columns = FALSE,
               name = "Mean(-log10(P))",col = colorRamp2(c(0, 2, 4, 6, 8), c("#F7FCF0", "#CCEBC5", "#7BCCC4", "#2B8CBE", "#084081")),
               width = ncol(int_mean)*unit(6, "mm"), 
               height = nrow(int_mean)*unit(6, "mm"),column_title = "INT cell type",
               cell_fun = text_fun)


####################################################################################
#AD
ad_mean <- mean_log_p[grep("^AD2022", rownames(mean_log_p)), 20:27]
ad_ldsc <- ldsc[grep("^AD2022", rownames(ldsc)), 20:27]
ad_magma <- magma[grep("^AD2022", rownames(magma)), 20:27]
rownames(ad_mean) <- gsub("[A-Za-z0-9]+-","", rownames(ad_mean))
colnames(ad_mean) <- gsub("Mean_Female_", "", colnames(ad_mean))
rownames(ad_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(ad_ldsc))
colnames(ad_ldsc) <- gsub("Mean_Female_", "", colnames(ad_ldsc))
rownames(ad_magma) <- gsub("[A-Za-z0-9]+-","", rownames(ad_magma))
colnames(ad_magma) <- gsub("Mean_Female_", "", colnames(ad_magma))
...
# 绘制热图
ad <- Heatmap(ad_mean[row_type_name,], cluster_rows = FALSE,cluster_columns = FALSE,
              name = "Mean(-log10(P))",col = colorRamp2(c(0, 2, 4, 6, 8), c("#F7FCF0", "#CCEBC5", "#7BCCC4", "#2B8CBE", "#084081")),
              width = ncol(ad_mean)*unit(6, "mm"), 
              height = nrow(ad_mean)*unit(6, "mm"),column_title = "AD cell type",
              cell_fun = text_fun)


####################################################################################
#oct
oct_mean <- mean_log_p[grep("^OCT", rownames(mean_log_p)), 20:27]
oct_ldsc <- ldsc[grep("^OCT", rownames(ldsc)), 20:27]
oct_magma <- magma[grep("^OCT", rownames(magma)), 20:27]
rownames(oct_mean) <- gsub("[A-Za-z0-9]+-","", rownames(oct_mean))
colnames(oct_mean) <- gsub("Mean_Female_", "", colnames(oct_mean))
rownames(oct_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(oct_ldsc))
colnames(oct_ldsc) <- gsub("Mean_Female_", "", colnames(oct_ldsc))
rownames(oct_magma) <- gsub("[A-Za-z0-9]+-","", rownames(oct_magma))
colnames(oct_magma) <- gsub("Mean_Female_", "", colnames(oct_magma))
...
# 绘制热图
oct <- Heatmap(oct_mean[row_type_name,], cluster_rows = FALSE,cluster_columns = FALSE,
               name = "Mean(-log10(P))",col = colorRamp2(c(0, 2, 4, 6, 8), c("#F7FCF0", "#CCEBC5", "#7BCCC4", "#2B8CBE", "#084081")),
               width = ncol(oct_mean)*unit(6, "mm"), 
               height = nrow(oct_mean)*unit(6, "mm"),column_title = "OCT cell type",
               cell_fun = text_fun)


####################################################################################
#an
an_mean <- mean_log_p[grep("^Anxiety", rownames(mean_log_p)), 20:27]
an_ldsc <- ldsc[grep("^Anxiety", rownames(ldsc)), 20:27]
an_magma <- magma[grep("^Anxiety", rownames(magma)), 20:27]
rownames(an_mean) <- gsub("[A-Za-z0-9]+-","", rownames(an_mean))
colnames(an_mean) <- gsub("Mean_Female_", "", colnames(an_mean))
rownames(an_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(an_ldsc))
colnames(an_ldsc) <- gsub("Mean_Female_", "", colnames(an_ldsc))
rownames(an_magma) <- gsub("[A-Za-z0-9]+-","", rownames(an_magma))
colnames(an_magma) <- gsub("Mean_Female_", "", colnames(an_magma))
...
# 绘制热图
an <- Heatmap(an_mean[row_type_name,], cluster_rows = FALSE,cluster_columns = FALSE,
              name = "Mean(-log10(P))",col = colorRamp2(c(0, 2, 4, 6, 8), c("#F7FCF0", "#CCEBC5", "#7BCCC4", "#2B8CBE", "#084081")), 
              width = ncol(an_mean)*unit(6, "mm"), 
              height = nrow(an_mean)*unit(6, "mm"),column_title = "AN cell type",
              cell_fun = text_fun)


####################################################################################
#HS
hs_mean <- mean_log_p[grep("^Hoarding", rownames(mean_log_p)), 20:27]
hs_ldsc <- ldsc[grep("^Hoarding", rownames(ldsc)), 20:27]
hs_magma <- magma[grep("^Hoarding", rownames(magma)), 20:27]
rownames(hs_mean) <- gsub("[A-Za-z0-9]+-","", rownames(hs_mean))
colnames(hs_mean) <- gsub("Mean_Female_", "", colnames(hs_mean))
rownames(hs_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(hs_ldsc))
colnames(hs_ldsc) <- gsub("Mean_Female_", "", colnames(hs_ldsc))
rownames(hs_magma) <- gsub("[A-Za-z0-9]+-","", rownames(hs_magma))
colnames(hs_magma) <- gsub("Mean_Female_", "", colnames(hs_magma))
...
# 绘制热图
hs <- Heatmap(hs_mean[row_type_name,], cluster_rows = FALSE,cluster_columns = FALSE,
              name = "Mean(-log10(P))",col = colorRamp2(c(0, 2, 4, 6, 8), c("#F7FCF0", "#CCEBC5", "#7BCCC4", "#2B8CBE", "#084081")),
              width = ncol(hs_mean)*unit(6, "mm"), 
              height = nrow(hs_mean)*unit(6, "mm"),column_title = "HS cell type",
              cell_fun = text_fun)

####################################################################################
#TS
ts_mean <- mean_log_p[grep("^Tourette", rownames(mean_log_p)), 20:27]
ts_ldsc <- ldsc[grep("^Tourette", rownames(ldsc)), 20:27]
ts_magma <- magma[grep("^Tourette", rownames(magma)), 20:27]
rownames(ts_mean) <- gsub("[A-Za-z0-9]+-","", rownames(ts_mean))
colnames(ts_mean) <- gsub("Mean_Female_", "", colnames(ts_mean))
rownames(ts_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(ts_ldsc))
colnames(ts_ldsc) <- gsub("Mean_Female_", "", colnames(ts_ldsc))
rownames(ts_magma) <- gsub("[A-Za-z0-9]+-","", rownames(ts_magma))
colnames(ts_magma) <- gsub("Mean_Female_", "", colnames(ts_magma))
...
# 绘制热图
ts <- Heatmap(ts_mean[row_type_name,], cluster_rows = FALSE,cluster_columns = FALSE,
              name = "Mean(-log10(P))",col = colorRamp2(c(0, 2, 4, 6, 8), c("#F7FCF0", "#CCEBC5", "#7BCCC4", "#2B8CBE", "#084081")),
              width = ncol(ts_mean)*unit(6, "mm"), 
              height = nrow(ts_mean)*unit(6, "mm"),column_title = "TS cell type",
              cell_fun = text_fun)


####################################################################################
#INS
ins_mean <- mean_log_p[grep("^Insomina", rownames(mean_log_p)), 20:27]
ins_ldsc <- ldsc[grep("^Insomina", rownames(ldsc)), 20:27]
ins_magma <- magma[grep("^Insomina", rownames(magma)), 20:27]
rownames(ins_mean) <- gsub("[A-Za-z0-9]+-","", rownames(ins_mean))
colnames(ins_mean) <- gsub("Mean_Female_", "", colnames(ins_mean))
rownames(ins_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(ins_ldsc))
colnames(ins_ldsc) <- gsub("Mean_Female_", "", colnames(ins_ldsc))
rownames(ins_magma) <- gsub("[A-Za-z0-9]+-","", rownames(ins_magma))
colnames(ins_magma) <- gsub("Mean_Female_", "", colnames(ins_magma))
...
# 绘制热图
ins <- Heatmap(ins_mean[row_type_name,], cluster_rows = FALSE,cluster_columns = FALSE,
               name = "Mean(-log10(P))",col = colorRamp2(c(0, 2, 4, 6, 8), c("#F7FCF0", "#CCEBC5", "#7BCCC4", "#2B8CBE", "#084081")),
               width = ncol(ins_mean)*unit(6, "mm"), 
               height = nrow(ins_mean)*unit(6, "mm"),column_title = "INS cell type",
               cell_fun = text_fun)


####################################################################################
#su
su_mean <- mean_log_p[grep("^Suicide", rownames(mean_log_p)), 20:27]
su_ldsc <- ldsc[grep("^Suicide", rownames(ldsc)), 20:27]
su_magma <- magma[grep("^Suicide", rownames(magma)), 20:27]
rownames(su_mean) <- gsub("[A-Za-z0-9]+-","", rownames(su_mean))
colnames(su_mean) <- gsub("Mean_Female_", "", colnames(su_mean))
rownames(su_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(su_ldsc))
colnames(su_ldsc) <- gsub("Mean_Female_", "", colnames(su_ldsc))
rownames(su_magma) <- gsub("[A-Za-z0-9]+-","", rownames(su_magma))
colnames(su_magma) <- gsub("Mean_Female_", "", colnames(su_magma))
...
# 绘制热图
su <- Heatmap(su_mean[row_type_name,], cluster_rows = FALSE,cluster_columns = FALSE,
              name = "Mean(-log10(P))",col = colorRamp2(c(0, 2, 4, 6, 8), c("#F7FCF0", "#CCEBC5", "#7BCCC4", "#2B8CBE", "#084081")),
              width = ncol(su_mean)*unit(6, "mm"), 
              height = nrow(su_mean)*unit(6, "mm"),column_title = "SU cell type",
              cell_fun = text_fun)


####################################################################################
#ED
ed_mean <- mean_log_p[grep("^Eating", rownames(mean_log_p)), 20:27]
ed_ldsc <- ldsc[grep("^Eating", rownames(ldsc)), 20:27]
ed_magma <- magma[grep("^Eating", rownames(magma)), 20:27]
rownames(ed_mean) <- gsub("[A-Za-z0-9]+-","", rownames(ed_mean))
colnames(ed_mean) <- gsub("Mean_Female_", "", colnames(ed_mean))
rownames(ed_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(ed_ldsc))
colnames(ed_ldsc) <- gsub("Mean_Female_", "", colnames(ed_ldsc))
rownames(ed_magma) <- gsub("[A-Za-z0-9]+-","", rownames(ed_magma))
colnames(ed_magma) <- gsub("Mean_Female_", "", colnames(ed_magma))
...
# 绘制热图
ed <- Heatmap(ed_mean[row_type_name,], cluster_rows = FALSE,cluster_columns = FALSE,
              name = "Mean(-log10(P))",col = colorRamp2(c(0, 2, 4, 6, 8), c("#F7FCF0", "#CCEBC5", "#7BCCC4", "#2B8CBE", "#084081")),
              width = ncol(ed_mean)*unit(6, "mm"), 
              height = nrow(ed_mean)*unit(6, "mm"),column_title = "ED cell type",
              cell_fun = text_fun)


####################################################################################
#ADHD
adhd_mean <- mean_log_p[grep("^ADHD", rownames(mean_log_p)), 20:27]
adhd_ldsc <- ldsc[grep("^ADHD", rownames(ldsc)), 20:27]
adhd_magma <- magma[grep("^ADHD", rownames(magma)), 20:27]
rownames(adhd_mean) <- gsub("[A-Za-z0-9]+-","", rownames(adhd_mean))
colnames(adhd_mean) <- gsub("Mean_Female_", "", colnames(adhd_mean))
rownames(adhd_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(adhd_ldsc))
colnames(adhd_ldsc) <- gsub("Mean_Female_", "", colnames(adhd_ldsc))
rownames(adhd_magma) <- gsub("[A-Za-z0-9]+-","", rownames(adhd_magma))
colnames(adhd_magma) <- gsub("Mean_Female_", "", colnames(adhd_magma))
...
# 绘制热图
adhd <- Heatmap(adhd_mean[row_type_name,], cluster_rows = FALSE,cluster_columns = FALSE,
                name = "Mean(-log10(P))",col = colorRamp2(c(0, 2, 4, 6, 8), c("#F7FCF0", "#CCEBC5", "#7BCCC4", "#2B8CBE", "#084081")),
                width = ncol(adhd_mean)*unit(6, "mm"), 
                height = nrow(adhd_mean)*unit(6, "mm"),column_title = "ADHD cell type",
                cell_fun = text_fun)

####################################################################################
#ASD
asd_mean <- mean_log_p[grep("^ASD", rownames(mean_log_p)), 20:27]
asd_ldsc <- ldsc[grep("^ASD", rownames(ldsc)), 20:27]
asd_magma <- magma[grep("^ASD", rownames(magma)), 20:27]
rownames(asd_mean) <- gsub("[A-Za-z0-9]+-","", rownames(asd_mean))
colnames(asd_mean) <- gsub("Mean_Female_", "", colnames(asd_mean))
rownames(asd_ldsc) <- gsub("[A-Za-z0-9]+-","", rownames(asd_ldsc))
colnames(asd_ldsc) <- gsub("Mean_Female_", "", colnames(asd_ldsc))
rownames(asd_magma) <- gsub("[A-Za-z0-9]+-","", rownames(asd_magma))
colnames(asd_magma) <- gsub("Mean_Female_", "", colnames(asd_magma))
...
# 绘制热图
asd <- Heatmap(asd_mean[row_type_name,], cluster_rows = FALSE,cluster_columns = FALSE,
               name = "Mean(-log10(P))",col = colorRamp2(c(0, 2, 4, 6, 8), c("#F7FCF0", "#CCEBC5", "#7BCCC4", "#2B8CBE", "#084081")),
               width = ncol(asd_mean)*unit(6, "mm"), 
               height = nrow(asd_mean)*unit(6, "mm"),column_title = "ASD cell type",
               cell_fun = text_fun)
