...
#====================================================================================================================
#SubType level
data <- read_tsv('02-All-cortex-subtype.txt')
# 3. 数据预处理
df <- data %>%
...
pdf("02-DiffCortex-Subtype.pdf", width = 9.50, height = 11.00)
...
#====================================================================================================================
#Type level
data <- read_tsv('02-All-cortex-type.txt')
# 3. 数据预处理
df <- data %>%
  filter(Trait %in% c("BD2025OConnel", "SCZ2022PGC3")) %>%
  mutate(logP = -log10(pvalue)) %>%
...
pdf("02-DiffCortex-Type.pdf", width = 9.50, height = 7.17)
grid.arrange(p1, p2, ncol = 1)
dev.off()